...
import type { QuoteAuthor } from "@/data/Authors"
...
const QuoteContentRow = ({
  ...
  const isAuthorTerminal = author.isTerminal
  ...
  return (
    <ContentRow
      classes={`quote-content-row ${left ? "left" : ""} ${
        hasImage || isAuthorTerminal || isAuthorUnknown
          ? "has-image"
          : "no-image"
      }`}
    >
      <div
        className={`quote-container ${
          author.theme ? author.theme : ""
            ...
            ${isOpen ? "open" : ""}`}
      >
        {none || !isOpen || isAuthorUnknown ? null : (
          <div className="author-box">
            {shrink ||
            nameOnly ||
            (!hasImage &&
              !isAuthorTerminal) ? null : isAuthorTerminal ? (
              <div className="blinkbox-container">
                <span className="floater tiny-code">
                  {number}
                </span>
                <span className="floater-under tiny-code">
                  {number2}
                </span>
                <BlinkingGrid />
              </div>
        ...
        <SimpleContainer classes="code-block quote-box">
          {transcription && (
            <button
              className="transcription-open"
              onClick={() => {
                openQuote()
              }}
            >
              Press to view transcription{" "}
              <FontAwesomeIcon
                icon={faArrowDown}
                className="blinking"
              />
