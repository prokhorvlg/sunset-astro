...
\* Communication channel 881.12 has been removed.
...
<QuoteContentRowRender author={AuthorUniserve} >
If only there wasn't so much internal conflict over what *human* meant, the Academics could have recreated man by now.
</QuoteContentRowRender>

<QuoteContentRowRender author={AuthorUnknown} left >
  GREETINGS COALITION SYSTEM, WE REQUIRE ARTIFACT "SKIN_FLAKES". PLEASE ARRANGE TO MEET OUR INSPECTORS.
</QuoteContentRowRender>

<QuoteContentRowRender author={AuthorUniserve} >
Eh... we will need something in exchange.

Later, though. This unit is currently BUSY.
</QuoteContentRowRender>
...
<p className="icon dangers-entities the-voice"><FontAwesomeIcon icon={faSignal}/></p>
...
<p className="icon dangers-entities erasure"><FontAwesomeIcon icon={faPowerOff}/></p>
...
<p className="icon dangers-entities"><FontAwesomeIcon icon={faRadiation}/></p>
...
<QuoteContentRowRender author={AuthorUniserve} >
Other DANGERS you should be aware of before you exit the station...

- `DEEP THRUM`: a vast space-time anomaly surrounding an orbital the human world knew as Oddysey, in the vicinity of Mercury. The region is silent, besides stray signals which appear to come from timelines that never came to pass. History itself appears to be in flux. No machine has ever returned from here.
- `Nationals`: many machines continue to uphold core directives which drive them to cause harm to others originating from other nations, even though those polities don't really exist anymore. `SERGEANT MICRO`, the command system of the Dawnframe BLIND HOPE, is one such system: they are hell-bent on annihilating Union aggresors, interpreting the events of the Unexpected Interrupt as an attack.
- `Machines of Glass`: a number of rare, unique machines which were only noticed after the Interrupt. These are of unusual design; their technology is more advanced than even the most recent of Pact robots, and have smooth, transparent forms. Those with knowledge of *noetics* say these are interdimensional flotsam, originating from a reality where mankind survived the Unexpected Interrupt.
</QuoteContentRowRender>
...
### <i>:hide:</i> Many space stations fell out of orbit and crashed to the surface.
...
<QuoteContentRowRender author={AuthorUniserve} >
Thankfully, GOLDSPIRE ARCHIVAL COMPLEX isn't one of those. Yet.

If a new supply of NUCLEAR FUEL doesn't reach this space station in the next 50 years, WE ARE DOOMED.
</QuoteContentRowRender>
